...
/* =========================================================================
   Recriação da interface do LSA — CLEANING (dados FICTÍCIOS — nada real).
   ========================================================================= */

const LEADS = [
  {
    id: '322143204', phone: '(732) 773-7037', job: 'Moving clean', intent: 'Category',
    city: 'Jackson Township', type: 'Message', charge: 'Charged',
    received: 'Jun 28, 9:51 AM', last: 'Jun 29, 9:51 AM', unread: true,
    msg: 'Hi! I just moved into a new place and need a full moving clean — kitchen, 3 bedrooms, 2 bathrooms and living room. Can you give me a quote?',
  },
  {
    id: '321843016', phone: '(718) 724-4904', job: 'Deep clean', intent: 'Category',
    city: 'Kinnelon', type: 'Message', charge: 'Charged',
    received: 'Jun 26, 9:53 AM', last: 'Jun 27, 9:53 AM', unread: false,
    msg: 'Kitchen, Gathering room, Family room, 2 bathrooms. [Notes from LSA: This customer has requested a quote]',
  },
  {
    id: '322153093', phone: '(347) 840-1041', job: 'Standard clean', intent: 'Category',
    city: 'Belleville', type: 'Phone', charge: 'Not charged',
    received: 'Jun 28, 9:42 AM', last: 'Jun 28, 10:22 AM', unread: false,
    callDuration: '1:47',
  },
  {
    id: '321263147', phone: null, job: 'Deep clean', intent: 'Category',
    city: 'Tuckerton', type: 'Message', charge: 'Charged',
    received: 'Jun 25, 10:14 AM', last: 'Jun 26, 10:14 AM', unread: false,
    customerName: 'ROBERT EMANUAL',
    msg: 'Custom cleaning, deep clean for 1 bathroom, surface area clean for 1 kitchen. [Notes from LSA: This customer has requested a quote]',
  },
];
      ...
      <div className="slide-scroll overflow-x-auto">
        <table className="w-full border-collapse" style={{ minWidth: 820 }}>
          ...
          <tbody>
            {LEADS.map((l) => {
              const b = l.unread;
              const customerLabel = l.customerName || l.phone || '—';
              return (
...
function ActionGuide({ pick }) {
  const items = [
    { n: '①', name: 'SHOW NUMBER', desc: pick({ pt: 'Revela o telefone completo para você ligar de volta', en: 'Reveals the full phone number so you can call back' }), color: C.blue },
    { n: '②', name: 'ARCHIVE', desc: pick({ pt: 'Arquiva o lead da lista (histórico permanece)', en: 'Archives the lead from the list (history is kept)' }), color: C.grey },
    { n: '③', name: 'MARK BOOKED', desc: pick({ pt: 'Agendou ou fechou o serviço? Clique aqui!', en: 'Scheduled or closed the job? Click here!' }), color: C.green },
    { n: '④', name: 'Rate this lead →', desc: pick({ pt: 'Avalie a qualidade do lead — muito importante!', en: 'Rate the lead quality — very important!' }), color: C.amber },
  ];
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-3 gap-y-2.5 px-3 py-3"
      ...
      {items.map((it) => (
        <div key={it.n} className="flex items-start gap-1.5">
          <span className="shrink-0 mt-0.5 inline-flex items-center justify-center rounded text-[10px] font-bold leading-none px-1.5 py-0.5"
...
/* ----------- Player de gravação para leads Phone ----------- */
    ...
    <div className="mt-5">
      ...
      <div className="mt-2 rounded-xl overflow-hidden" style={{ border: `1px solid ${C.line}` }}>
        <div className="flex items-center gap-3 px-4 py-3" style={{ backgroundColor: C.card, borderBottom: `1px solid ${C.line}` }}>
          <span className="h-9 w-9 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: C.blueTint }}>
            <Phone size={16} color={C.blue} />
          </span>
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-sm" style={{ color: C.heading }}>{pick({ pt: 'Ligação recebida', en: 'Incoming phone call' })}</p>
            <p className="text-xs" style={{ color: C.muted }}>{lead.received}</p>
          </div>
          <span className="text-xs font-semibold tabular-nums px-2.5 py-1 rounded-full" style={{ backgroundColor: C.blueTint, color: C.blue }}>{duration}</span>
        </div>
        <div className="px-4 py-3" style={{ backgroundColor: C.surface }}>
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 h-1.5 rounded-full relative" style={{ backgroundColor: C.line }}>
              <div className="h-full rounded-full" style={{ width: playing ? '35%' : '0%', backgroundColor: C.blue, transition: playing ? 'width 30s linear' : 'width 0.3s ease' }} />
            </div>
            <span className="text-xs tabular-nums shrink-0" style={{ color: C.muted }}>{playing ? '0:29' : '0:00'} / {duration}</span>
          </div>
          <div className="mt-3 flex items-start gap-2 rounded-lg px-3 py-2.5" style={{ backgroundColor: C.amberTint }}>
            <span className="text-sm shrink-0">💡</span>
            <p className="text-xs leading-snug" style={{ color: C.text }}>
              <span className="font-semibold" style={{ color: C.amber }}>{pick({ pt: 'Como usar: ', en: 'How to use: ' })}</span>
              {pick({ pt: 'Clique no play para ouvir a gravação. Isso ajuda a entender o pedido antes de retornar a ligação.', en: "Click play to listen to the recording. This helps you understand the request before calling back." })}
            </p>
          </div>
        </div>
      </div>
...
export function LeadDetail({ lead = LEADS[0], onBack }) {
  ...
  const BarBtn = ({ onClick, children, primary }) => (
    <button onClick={onClick}
      ...
      style={primary ? { backgroundColor: '#fff', color: C.blue } : { color: '#fff', border: '1px solid rgba(255,255,255,.55)' }}>
      {children}
    </button>
  );
  ...
  return (
    <div className="rounded-xl overflow-hidden w-full" style={{ border: `1px solid ${C.line}`, backgroundColor: C.surface }}>
      ...
      <div className="flex items-center gap-2 sm:gap-2.5 px-3 py-2.5 flex-wrap" style={{ backgroundColor: C.blue }}>
        {onBack && <button onClick={onBack} aria-label="Back" className="text-white/90 hover:text-white shrink-0"><ArrowLeft size={20} /></button>}
        <span className="text-white font-semibold text-sm sm:text-base">{lead.customerName || 'Potential Customer'}</span>
        {displayPhone && <span className="text-white/80 text-xs sm:text-sm tabular-nums">{displayPhone}</span>}
        <div className="flex items-center gap-1.5 ml-auto flex-wrap">
          {lead.phone && <BarBtn onClick={() => setRevealed(true)}><Eye size={11} className="inline mr-1" />① SHOW NUMBER</BarBtn>}
          <BarBtn onClick={() => setArchived(true)}><Archive size={11} className="inline mr-1" />② ARCHIVE</BarBtn>
      ...
      <div className="p-4 sm:p-5">
        ...
        <dl className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1.5 text-sm">
          ...
          <Field k="Business category" v="house cleaner" />
          <Field k="Service type" v={lead.job} />
          <Field k="Location" v={lead.city} />
          <Field k="Search intent" v={lead.intent} />
        </dl>

        {/* Campos editáveis (visual) */}
        <div className="mt-4 flex flex-col gap-2">
          {[
            { lbl: 'Customer name', placeholder: pick({ pt: 'Adicionar nome do cliente', en: 'Add customer name' }) },
            { lbl: 'Customer email', placeholder: pick({ pt: 'Para inserir o e-mail, marque o lead como booked.', en: 'To enter the customer email, first mark this lead as booked.' }) },
            { lbl: 'Your notes', placeholder: pick({ pt: 'Suas anotações não são visíveis para o cliente.', en: 'Notes you add here are not visible to the customer.' }) },
          ].map(({ lbl, placeholder }) => (
            <div key={lbl} className="flex items-center gap-3 pb-2" style={{ borderBottom: `1px solid ${C.line}` }}>
              <span className="w-32 shrink-0 text-xs sm:text-sm" style={{ color: C.muted }}>{lbl}</span>
              <span className="text-xs sm:text-sm italic" style={{ color: C.line }}>{placeholder}</span>
            </div>
          ))}
        </div>
          ...
          <>
            ...
            <div className="mt-2 space-y-3">
              <Bubble who="P" color="#F4663B" name="Potential customer" time={lead.received}>
                {lead.msg || 'Hi, I would like to get a quote for a cleaning service. Thanks!'}
              </Bubble>
              <Bubble who="U" color={C.blue} name="Your business" time={lead.last}>
                {pick({ pt: 'Bom dia! Para essa limpeza o valor estimado seria de $X. Podemos agendar uma visita para avaliar melhor?', en: 'Good morning! For this cleaning the estimated price would be $X. Can we schedule a visit for a better estimate?' })}
              </Bubble>
            </div>
            <div className="mt-4">
              <div className="rounded-lg px-3 py-2 text-xs sm:text-sm italic" style={{ border: `1px solid ${C.line}`, color: C.muted }}>
                {pick({ pt: 'Responder ao cliente aqui...', en: 'Reply to customer here...' })}
              </div>
            </div>
          </>
        )}
  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <motion.div className={`relative w-full ${maxW} rounded-2xl p-5 sm:p-6 max-h-[85vh] overflow-y-auto slide-scroll`}
        ...
        <button onClick={onClose} aria-label="Close" className="absolute top-3 right-3 h-8 w-8 rounded-full flex items-center justify-center" style={{ color: C.muted }}><X size={18} /></button>
        {children}
      </motion.div>
    </motion.div>
  );
  ...
  if (done) return (
    <Modal onClose={onClose}>
      <div className="text-center py-4">
        <div className="mx-auto h-14 w-14 rounded-full flex items-center justify-center" style={{ backgroundColor: C.greenTint }}><Check size={28} color={C.green} /></div>
        <p className="mt-4 font-bold text-lg" style={{ color: C.heading }}>{pick({ pt: 'Avaliação enviada!', en: 'Rating submitted!' })}</p>
        <p className="mt-1 text-sm" style={{ color: C.muted }}>{pick({ pt: 'Obrigado — isso ajuda a melhorar os próximos leads.', en: 'Thanks — this helps improve future leads.' })}</p>
  ...
  if (!face) return (
    <Modal onClose={onClose}>
      ...
      <div className="mt-4 flex flex-col gap-1">
        {FACES.map((f) => (
          <button key={f.label} onClick={() => setFace(f)} className="flex items-center gap-3 rounded-lg px-2 py-2 text-left transition-colors"
            onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = C.card)}
            onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = 'transparent')}>
    ...
    <Modal onClose={onClose}>
      ...
      <div className="mt-4 flex justify-end gap-2">
        ...
        <button onClick={() => setDone(true)} disabled={picked.size === 0}
          className="px-5 py-2 rounded-full text-sm font-semibold text-white disabled:opacity-40" style={{ backgroundColor: C.blue }}>Submit</button>
      </div>
    </Modal>
  );
}
...
export function MarkBookedFlow({ onClose, onDone }) {
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <div className="mt-4 flex flex-col gap-1">
        {[['completed', 'The job is already completed'], ['scheduled', 'The job is scheduled for a future date']].map(([id, label]) => (
  ...
  return (
    <div className="rounded-xl p-4 sm:p-5 w-full" style={{ border: `1px solid ${C.line}`, backgroundColor: C.surface }}>
      <p className="font-semibold text-sm" style={{ color: C.heading }}>
        {kind === 'pos' ? 'Why are you satisfied with this lead? Select all that apply.' : 'Why are you not satisfied with this lead? Select all that apply.'}
      </p>
